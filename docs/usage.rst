...
.. note::
   ...
   *write* a plain C++ or compiled-Lua plugin itself: ``state::begin_plugin`` and
   ...
   :doc:`Writing Plugins<writing_plugins>`) and built as a native shared library; Python (like
   any other consumer) only ever *loads* the result via ``grunk.plugin.load_native``.

See :doc:`Writing Plugins<writing_plugins>` for how to actually author one of the three kinds above
- by hand, or, for a plain C++ plugin wrapping an existing library, generated from its own headers
via ``grunk.codegen``.
